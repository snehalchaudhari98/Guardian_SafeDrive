...
public class CrimeHostspotFragment extends FragmentActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Places.initialize(getApplicationContext(), "AIzaSyDiFPvdgShaacPmuEQywoxHxXRrhDvMioU");

        // Initialize the Places Client
        PlacesClient placesClient = Places.createClient(this);

        AutocompleteSupportFragment startAddressFragment = (AutocompleteSupportFragment)
                getSupportFragmentManager().findFragmentById(R.id.startAddressEditText);
        startAddressFragment.setPlaceFields(Arrays.asList(Place.Field.ID, Place.Field.NAME, Place.Field.ADDRESS));

        AutocompleteSupportFragment endAddressFragment = (AutocompleteSupportFragment)
                getSupportFragmentManager().findFragmentById(R.id.endAddressEditText);
        endAddressFragment.setPlaceFields(Arrays.asList(Place.Field.ID, Place.Field.NAME, Place.Field.ADDRESS));

//        startAddressFragment.getView().setBackgroundColor(Color.WHITE);
//        endAddressFragment.getView().setBackgroundColor(Color.WHITE);
        ...
        // Set onClickListener for the "Show Route" button
        showRouteButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                // Perform the Directions API request
                new GetDirectionsTask().execute(startAddress, endAddress);
            }
        });
        ...
        endAddressFragment.setOnPlaceSelectedListener(new PlaceSelectionListener() {
            @Override
            public void onPlaceSelected(@NonNull Place place) {
    ...
    private class GetDirectionsTask extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            try {
                ...
                synchronized(this) {
                    startLatLng = getLatLngFromAddress(startAddress);
                    endLatLng = getLatLngFromAddress(endAddress);
                }
            ...
            if (result != null) {
                ...
                try {
                    ...
                    if (routes.length() > 0) {
                        ...
                        // Draw the original route on the map
                        ...
                        LatLng startLatLng = decodedPoints.get(0);
                        LatLng endLatLng = decodedPoints.get(decodedPoints.size() - 1);
...
//        }
        ...
        private void drawRoute(List<LatLng> decodedPoints) {
    ...
    private LatLngBounds getSquareBounds(LatLng center, double radius) {
